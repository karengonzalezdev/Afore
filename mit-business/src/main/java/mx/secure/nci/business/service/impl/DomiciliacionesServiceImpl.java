...
import mx.secure.nci.business.util.ConsumeWS;
import mx.secure.aportaciones.ws.utils.UtilsParseXML;
import mx.secure.nci.business.bean.RespGeneracionArchivosDomi;
import mx.secure.nci.business.envelop.ClientesEnvelope;
...
import mx.secure.nci.persistence.DomiciliacionesPersistence;
import mx.secure.nci.persistence.DomisBitacoraPersistence;
...
public class DomiciliacionesServiceImpl implements IDomiciliacionesService{
	...
	public Integer getConsecutivoArchivoDomiLotes (String tipoArchivo, String idAportacionOrigen) throws Exception {
		...
		try {
			...
			if (tipoArchivo.equals("459")) { // secure
				
				if (idAportacionOrigen.equals("0")) { // Domiciliaciones ordinarias
					consecutivo = this.archivoDomiPersistence.consultarConsecutivosecureOrdinario();
				} else { // Domiciliaciones traspasos
					consecutivo = this.archivoDomiPersistence.consultarConsecutivosecureTraspasos();
				}
	...
	/*public Integer getConsecutivoArchivoDomiLotes (String tipoArchivo, String bancoCuentas) throws Exception {
		...
		try {
			...
			if (tipoArchivo.equals("459")) { // secure
				...
				if (bancoCuentas.equals("459")) { // Cuentas secure
					consecutivo = this.archivoDomiPersistence.consultarConsecutivosecureOrdinario();
				} else { // Cuentas diferentes a secure 
					consecutivo = this.archivoDomiPersistence.consultarConsecutivosecureTraspasos();
				}
			} else if (tipoArchivo.equals("460")) { // BBVA
	...
	public String getNombreArchivoDomiLotes (Integer consecutivo, String tipoArchivo) throws Exception {
		...
		try {
			...
			if (tipoArchivo.equals("459")) { // secure
				fecha = this.archivoDomiPersistence.consultarFechaFormatoDomisecure();
	...
	public String getNombreArchivoDomi (Short idOrigenAportacion, Short type, Integer consecutivo, Date fecha) throws Exception {
		...
		try {
			...
			if( ID_secure_NCI.equals(type) ) {
				sdf = new SimpleDateFormat(DOMI_FILES_DATE_FORMAT_secure);
				
				fileName.append(ID_secure_DOMICILIACIONES).append(sdf.format(fecha))
		...
		try {
			...
			try {
				if(ID_secure_NCI.equals(solicitudFilter.getIdTipoArchivo())){									  
					bmxRecords =  this.mapVOTosecureRecordBeanF4(domis, consecutivo, fecha, idArchivo);
	...
	private secureDomiFixLenghtFileRecordBeanPar  mapVOTosecureRecordBeanF4(List<SolicitudVO> solicitudes,Integer consecutivo, Date fecha, Long idArchivo) {
		...
		try {
			...
			for(SolicitudVO sol :solicitudes){
				try{
					records = this.getsecureRecordsF4(sol,fecha);
	...
	private BancomerDomiFixLenghtFileRecordBeanPar generaBeanArchivoBancomer( Date fecha
		...
		try {
			...
			LOGGER.error("ERROR UBICACIÓN     :" + DomiciliacionesServiceImpl.class.getCanonicalName());
			LOGGER.error("ERROR METODO        :" + "generaBeanArchivosecure");
			LOGGER.error("ERROR MENSAJE       :" + e.getMessage());
			LOGGER.error("ERROR DESCRIPCIÓN   :", e);
			return null;
		} finally {
