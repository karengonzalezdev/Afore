package mx.secure.nci.business.wrapped;

import java.util.List;

import mx.secure.nci.business.vo.SolicitudVO;

public class PreSolicitudFilter extends SolicitudVO{
	private List<Short> estatusSolicitudes;
	private Short idEstatusSolicitudOld;

	public List<Short> getEstatusSolicitudes() {
		return estatusSolicitudes;
	}

	public void setEstatusSolicitudes(List<Short> estatusSolicitudes) {
		this.estatusSolicitudes = estatusSolicitudes;
	}

	public Short getIdEstatusSolicitudOld() {
		return idEstatusSolicitudOld;
	}

	public void setIdEstatusSolicitudOld(Short idEstatusSolicitudOld) {
		this.idEstatusSolicitudOld = idEstatusSolicitudOld;
	}



}
