...
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_CIF_APV_SELCC_ENVIADO_CIF;
...
public class MovimientosCifWebServiceImpl implements IMovimientosCifWebService {
	...
	@Autowired secureCIFPersistence  cifPeristence;
...
//		cifService.consultarsecure(new secure_CIFFilter());
...
//		data.addAll(getDummyData(1));
	...
	public List<secure_CIFVo> send2CIF(List<secure_CIFVo> vos,String usuario) throws MitBusinessException {
		List<secure_CIFVo> rejected = new ArrayList<secure_CIFVo>();
		for(secure_CIFVo vo:vos) {
			try {
				Integer ok= null;
				ok=cifService.insertarsecure(vo);
				if(ok > 0) {
...
//					}
	...
	public ReporteVO generaReporte(List<secure_CIFVo> dataReportBean) throws MitBusinessException {
		ReporteVO repoBean = null;
		try{
			BasicXLSParamReportBean<secure_CIFVo> paramBean = new BasicXLSParamReportBean<secure_CIFVo>();
	...
	public ReporteVO generaReportePrev(List<secure_CIFVo> dataReportBean) throws MitBusinessException {
		ReporteVO repoBean = null;
		try{
			BasicXLSParamReportBean<secure_CIFVo> paramBean = new BasicXLSParamReportBean<secure_CIFVo>();
	...
	public List<secure_CIFVo> getDataSended(List<secure_CIFVo> sended) throws MitBusinessException{
		List<secure_CIFVo> prf_sended = new ArrayList<secure_CIFVo>();
		try{
			for(secure_CIFVo vo:sended) {
				secure_CIFFilter f = new secure_CIFFilter(vo.getIdItem());
				prf_sended.addAll(cifService.consultarsecure(f));
			}
		} catch(Exception ex){
			MitBusinessException mitBusinessException = new MitBusinessException(GenerateExceptionDetails.generate(
