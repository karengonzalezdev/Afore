...
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_ORIGEN_APORTACION_VENTANILLA;
...
import mx.secure.nci.web.service.IValidacionManualWebService;
...
import secure.mx.iib.nci.identificacioncliente.clienteservice.v1.types.InformacionCliente;
...
import secure.mx.iib.nci.matrizconvivencia.matrizconvivenciaservice.v1.types.Motivo;
...
public class ValidacionManualWebServiceImpl implements IValidacionManualWebService {
	...
	public List<BancosBean> consultar(BancosFilterBean bancosFilter) throws MitBusinessException {
		...
		try {
			...
			for (BancosVO bancosVO : bancosList) {
				...
				if(bancosVO.getSubctasecure() != null)
				{
					GenericCatalogoBean subcuenta = new GenericCatalogoBean();
					subcuenta.setValor(bancosVO.getSubctasecure().getDescripcionItgy());
					bancosBean.setSubctasecure(subcuenta);
				}
	...
	public List<GenericCatalogoBean> obtenersecure() throws MitBusinessException {
		List<GenericCatalogoBean> listasecure = new ArrayList<GenericCatalogoBean>();
		try {
			for(MapaReferenciasVO maps : mapaReferenciasService.consultar())
			{
				GenericCatalogoBean secure = new GenericCatalogoBean();
				secure.setId(maps.getClaveNCI());
				secure.setValor(maps.getDescripcionItgy());
				listasecure.add(secure);
			}
		} catch (final Exception ex) {
			final MitBusinessException mitBusinessException = new MitBusinessException(
					GenerateExceptionDetails.generate(ErrorCodeService.EX_EXCEPTION, "Al Consultar catalogo ",
							new Object[] { "BancosServiceImpl", "obtenersecure()" }, ex));
...
//			bancosVo.setNumCuentaIndividual(bancosBean.getNumCuentaIndividual());
			...
			mapaReferencias.setClaveNCI(bancosBean.getSubctasecure().getId());
			bancosVo.setSubctasecure(mapaReferencias);
			if(idEmpresa!=null){
...
//			subcta.setId((short) bancosBean.getSubctasecure().getId());
//			conciliacionVO.setReferenciaSubcuentasecure(Integer.valueOf(bancosBean.getOrigenAportacion().getId()));
			if(bancosBean.getSubctasecure() != null && bancosBean.getSubctasecure().getId()!=null) {
				conciliacionVO.setReferenciaSubcuentasecure(new BigInteger(bancosBean.getSubctasecure().getId().toString()));
			}
			if(idEmpresa!=null) {
			...
			conciliacionVO.setFechaPagosecure(bancosBean.getFechaDeposito());
