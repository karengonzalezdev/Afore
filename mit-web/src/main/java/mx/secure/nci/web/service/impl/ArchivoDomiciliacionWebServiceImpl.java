...
import static mx.secure.nci.business.util.ConstantesCatalogos.TXT_DOMICILIACIONES_MAY;
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_ESTATUS_TRASPASO_PENDIENTE;
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_PENDIENTE;
...
import mx.secure.nci.business.service.IIdentificarClienteService;
import mx.secure.nci.business.service.IIndicadoresService;
...
import mx.secure.nci.web.beans.ArchivoDomiParamRegistry;
import mx.secure.nci.web.beans.ArchivoDomiViewParamBean;
import mx.secure.nci.web.beans.CifrasBancoBean;
...
import mx.secure.nci.web.beans.traspasos.DetalleMesDetalleBean;
import mx.secure.nci.web.beans.traspasos.InversionDomiTraspasosBean;
import mx.secure.nci.web.beans.traspasos.InversionDomiTraspasosDetalleBean;
import mx.secure.nci.web.beans.traspasos.MonitoreoCargosDomiBean;
import mx.secure.nci.web.beans.traspasos.MonitoreoCargosDomiDetalleBean;
import mx.secure.nci.web.beans.traspasos.MonitoreoSolicitudesDetalleBean;
import mx.secure.nci.web.service.IArchivoDomiciliacionWebService;
import mx.secure.nci.web.util.UtilMapping;
...
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.Catalogo;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.DomiciliacionPortType;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.ActualizarSolicitudReq;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.ActualizarSolicitudResp;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.ActualizarTraspasoReq;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.ActualizarTraspasoResp;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.Domiciliacion;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.Frecuencia;
import secure.mx.iib.secure.domiciliaciones.domiciliacionservice.v1.types.Solicitud;
import secure.mx.iib.nci.identificacioncliente.clienteservice.v1.types.ConsultarClienteBasicoResp;
import secure.mx.iib.nci.identificacioncliente.clienteservice.v1.types.Correo;
import secure.mx.iib.nci.identificacioncliente.clienteservice.v1.types.ListaCorreos;
import secure.mx.iib.nci.identificacioncliente.clienteservice.v1.types.ListaTelefonos;
import secure.mx.iib.nci.identificacioncliente.clienteservice.v1.types.Telefono;
...
//		domiTraspasosService.actualizaDomiDiversificacion(bean.getClaveSolicitud(), ConstantesCatalogos.IDsecure_CP, bean.getCpDosMeses());
//		domiTraspasosService.actualizaDomiDiversificacion(bean.getClaveSolicitud(), ConstantesCatalogos.IDsecure_LP, bean.getLpDoceMeses());
//		domiTraspasosService.actualizaDomiDiversificacion(bean.getClaveSolicitud(), ConstantesCatalogos.IDsecure_AVPILP, bean.getAvpilp());
//		domiTraspasosService.actualizaDomiDiversificacion(bean.getClaveSolicitud(), ConstantesCatalogos.IDsecure_ACR, bean.getAcr());
...
//			resp.getMensajes().getMensaje().get(0).getCodigo();
	...
	public boolean actualizaDiversificacion(String cveSolicitud, BigDecimal cp, BigDecimal lp, BigDecimal avpilp, BigDecimal acr, BigDecimal totalDiver, String usuario) throws MitBusinessException{
		...
		Integer total = 0;
		List<secureFilter> lstInsert = new ArrayList<secureFilter>(); 
		...
		if(cp!=null && cp.compareTo(new BigDecimal("0"))==1){
			...
			secureFilter regCp = new secureFilter();
			regCp.setFondosecure(ConstantesCatalogos.IDsecure_CP);
			regCp.setMonto(cp);
			regCp.setPorcentaje(cpPor.intValue());
			
			lstInsert.add(regCp);
			total += cpPor.intValue();
//			domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, ConstantesCatalogos.IDsecure_CP, cp, cpPor.intValue(), usuario, 1);
		}
		if(lp!=null && lp.compareTo(new BigDecimal("0"))==1){
			...
			secureFilter regLp = new secureFilter();
			regLp.setFondosecure(ConstantesCatalogos.IDsecure_LP);
			regLp.setMonto(lp);
			regLp.setPorcentaje(lpPor.intValue());

			lstInsert.add(regLp);
			total += lpPor.intValue();
//			domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, ConstantesCatalogos.IDsecure_LP, lp, lpPor.intValue(), usuario, 1);
		}
		if(avpilp!=null && avpilp.compareTo(new BigDecimal("0"))==1){
			...
			secureFilter regAv = new secureFilter();
			regAv.setFondosecure(ConstantesCatalogos.IDsecure_AVPILP);
			regAv.setMonto(avpilp);
			regAv.setPorcentaje(avpilpPor.intValue());

			lstInsert.add(regAv);
			total += avpilpPor.intValue();
//			domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, ConstantesCatalogos.IDsecure_AVPILP, avpilp, avpilpPor.intValue(), usuario, 1);
		}
		if(acr!=null && acr.compareTo(new BigDecimal("0"))==1){
			...
			secureFilter regAc = new secureFilter();
			regAc.setFondosecure(ConstantesCatalogos.IDsecure_ACR);
			regAc.setMonto(acr);
			regAc.setPorcentaje(acrPor.intValue());

			lstInsert.add(regAc);
			total += acrPor.intValue();
//			domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, ConstantesCatalogos.IDsecure_ACR, acr, acrPor.intValue(), usuario, 1);
		}
		
		for(int i=0;i<lstInsert.size();i++){
			secureFilter reg = lstInsert.get(i);
			if(i==lstInsert.size()-1){
				if(total==100){
					domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, reg.getFondosecure(), reg.getMonto(), reg.getPorcentaje(), usuario, 1);
				}else if(total<100){
					domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, reg.getFondosecure(), reg.getMonto(), reg.getPorcentaje()+(100-total), usuario, 1);
				}else{
					domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, reg.getFondosecure(), reg.getMonto(), reg.getPorcentaje()+(total-100), usuario, 1);
				}
			}else{
				domiTraspasosService.insertarDomiDiversificacion(cveSolicitud, reg.getFondosecure(), reg.getMonto(), reg.getPorcentaje(), usuario, 1);
			}
