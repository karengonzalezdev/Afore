...
import mx.secure.nci.web.service.IConsultaDomiciliacionWebService;
...
//					solicitudBean.setUsuarioActualizacion(solicitudVO.getUsuarioActualizacion());
				...
				if(solicitudVO.getFondosecure() != null){
					GenericCatalogoBean fondosecure = new GenericCatalogoBean();
					UtilMapping.mapVoToBean(fondosecure, solicitudVO.getFondosecure());
					solicitudBean.setFondosecure(fondosecure);
				}
	...
	public ReporteVO generaReporteDetallado(List<DiversificacionBean> lstDiversificacion, List<ValidacionesBean> lstValidacion, 
		...
		public DetalleSpeisecure obtenerDetalleSpei(String folio) throws MitBusinessException{
			DetalleSpeisecure reg = new DetalleSpeisecure();
			SolicitudFilter solFilter = new SolicitudFilter();
			solFilter.setFolio(folio);
			
			List<SpeiDetallesecureVO> reto = solicitudService.consultaDetalleSpei(solFilter);
		...
		public DetalleSpeisecure obtenerDetalleNomina(String folio, Long numCuentaIndividual) throws MitBusinessException{
			...
			List<SpeiDetallesecureVO> reto = solicitudService.consultaDetalleNomina(solFilter);
			for(int i=0;i<reto.size();i++){
