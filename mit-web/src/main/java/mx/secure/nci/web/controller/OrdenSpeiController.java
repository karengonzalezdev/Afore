 ...
 * 
 */
package mx.secure.nci.web.controller;

import static mx.secure.nci.business.util.ConstantesCatalogos.ID_ESTATUS_FINALIZADO;
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_ETAPA_NCI_OPERACION;
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_RESULTADO_SIN_HALLAZGOS;
import static mx.secure.nci.business.util.ConstantesCatalogos.ID_SUBETAPA_CONCILIACION;
...
import mx.secure.nci.business.service.IBitacoraProcesoService;
...
import mx.secure.nci.web.beans.ClienteOrdenSpeiBean;
import mx.secure.nci.web.beans.FondoBean;
...
public class OrdenSpeiController implements Serializable{
	...
	public Boolean guardarOrdenSpei(ClienteOrdenSpeiBean clienteSpei, boolean monto) {
		...
		GenericCatalogoVO fondosecure;
		for (FondoBean fondo : clienteSpei.getOrdenSpei().getDiversificacionSpei()) {
			if(fondo.isSelecion()){
				fondosecure = new GenericCatalogoVO();
				div = new DiversificacionOrdenesVO();
				fondosecure.setId(fondo.getId());
				fondosecure.setValor(fondo.getValor().replaceAll("\\$","").replaceAll("\\%","").replaceAll("\\,", ""));
				div.setFondosecure(fondosecure);
